"""Development Control Center v2 UI.

This keeps the user-facing lifecycle in one place while leaving each repo's
actual SYNC / RUN / BUILD / UPDATE logic in its tracked formal scripts.
"""
...
class App(ttk.Frame):
    ...
    def _build(self) -> None:
        ...
        repo_buttons.columnconfigure((0, 1), weight=1)
        ttk.Button(repo_buttons, text="状態更新", command=self.refresh).grid(row=0, column=0, sticky="ew", padx=(0, 3))
        ...
        new_buttons.columnconfigure((0, 1), weight=1)
        ...
        update_buttons.columnconfigure((0, 1), weight=1)
        ...
        flow = ttk.LabelFrame(right, text="STARTUP SET → PR/CI → SYNC → RUN → BUILD → UPDATE", padding=10)
        flow.grid(row=4, column=0, sticky="ew", pady=(0, 10))
        for col in range(6):
            flow.columnconfigure(col, weight=1)
        ttk.Button(flow, text="STARTUP SETコピー", command=self.copy_startup_set).grid(row=0, column=0, sticky="ew", padx=(0, 4))
        ttk.Button(flow, text="PR / CI更新", command=self.refresh_github).grid(row=0, column=1, sticky="ew", padx=4)
        self.sync_button = ttk.Button(flow, text="③ SYNC", command=lambda: self.launch("sync"))
        self.sync_button.grid(row=0, column=2, sticky="ew", padx=4)
        self.run_button = ttk.Button(flow, text="④ RUN_DEV", command=lambda: self.launch("run"))
        self.run_button.grid(row=0, column=3, sticky="ew", padx=4)
        self.build_button = ttk.Button(flow, text="⑤ BUILD", command=lambda: self.launch("build"))
        self.build_button.grid(row=0, column=4, sticky="ew", padx=4)
        self.release_button = ttk.Button(flow, textvariable=self.release_button_var, command=lambda: self.launch("release"))
        self.release_button.grid(row=0, column=5, sticky="ew", padx=(4, 0))
        ...
        ttk.Label(flow, textvariable=self.candidate_source_var).grid(row=1, column=5, sticky="w", padx=(8, 0), pady=(10, 0))
        ttk.Label(flow, text="expected branch HEADがCI greenならcandidate SHAを自動入力。工程は自動連続しません。").grid(
        ...
        for row, (label, var) in enumerate([
            ...
            ttk.Label(entries, textvariable=var, wraplength=780).grid(row=row, column=1, sticky="w")
    ...
    def refresh(self) -> None:
        ...
        self.banner_var.set(
            "正式repo / branch / origin / tracked clean を確認済み。"
            if state.safe_for_lifecycle(self.current)
            else "安全条件を満たしていないためライフサイクル操作を停止中。"
        )
    ...
    def refresh_github(self) -> None:
        ...
        self.ci_var.set(f"{state.ci_state}   checks={state.check_count}")
        if state.latest_pr:
        ...
        if state.candidate_ready:
            ...
            if not current_value or current_value == old_auto:
                ...
                self._log(f"{self.current.name}: CI greenのbranch HEADをcandidateへ自動反映 {short_sha(state.branch_sha)}")
    ...
    def _candidate_changed(self) -> None:
        ...
        self.candidate_source_var.set("AUTO / CI green" if value and value == auto_value else ("MANUAL" if value else "-"))
    ...
    def copy_startup_set(self) -> None:
        ...
        self._log(f"{self.current.name}: STARTUP SETをコピーしました。ChatGPTは開きません。")
        messagebox.showinfo("STARTUP SET", "現在のChatGPTへ貼り付けるSTARTUP SETをコピーしました。")
    ...
    def setup_new_repo(self) -> None:
        ...
        self._log(f"{remote.name}: clone確認 + 管理登録STARTUP指示をコピーしました。")
        messagebox.showinfo(
            "新規repo",
            "正式ローカルrepoを確認しました。\n管理登録・標準入口整備のSTARTUP指示をコピーしました。\n現在のChatGPTへ貼り付けてください。",
        )
    ...
    def check_self_update(self) -> None:
        ...
        if github_state.error:
            ...
            self.self_update_var.set(f"最新版 {short_sha(local_state.head)}")
        elif github_state.ci_state != "GREEN":
            self.self_update_var.set(f"候補 {short_sha(github_state.branch_sha)} / CI {github_state.ci_state}")
        else:
            self.self_update_sha = github_state.branch_sha
            self.self_update_var.set(f"更新あり → {short_sha(self.self_update_sha)}")
        self._set_button_states()
    ...
    def apply_self_update(self) -> None:
        ...
        if not messagebox.askyesno(
            "Control Center更新",
            f"development-managementを {short_sha(self.self_update_sha)} へSYNCします。\n成功後はControl Centerを開き直してください。",
        ):
            return
        ...
        if completed.returncode != 0:
            ...
            return
        messagebox.showinfo("更新完了", "Control Centerの更新が完了しました。いったん閉じて開き直してください。")
        self.self_update_var.set(f"更新済み {short_sha(self.self_update_sha)} / 再起動してください")
        self.self_update_sha = ""
        self._set_button_states()
    ...
    def _set_button_states(self) -> None:
        ...
        if not safe or not self.entrypoints or busy:
            ...
            self.sync_button.configure(state="normal" if self.entrypoints.sync.ready and candidate_sha_is_valid(self.candidate_var.get()) else "disabled")
