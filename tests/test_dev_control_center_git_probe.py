...
from tools.ai_orchestrator.common import CommandResult, OrchestratorError

BASE = 'a' * 40


def result(rc=0, stdout='', stderr=''):
    return CommandResult(('git',), rc, stdout, stderr)
...
class WorktreeHeadProbeTests(unittest.TestCase):
    def setUp(self):
        ...
        for module in (o, common):
            self.enterContext(mock.patch.object(module, 'run_streaming', self.runner))
            self.enterContext(mock.patch.object(module, 'resolved_command', return_value=['git']))
        self.sleep = self.enterContext(mock.patch.object(o.time, 'sleep'))

    def check(self):
        o.assert_agent_did_not_commit(self.worktree, BASE)
    ...
    def test_persistent_empty_error_stops_after_three_attempts(self):
        ...
        with self.assertRaisesRegex(OrchestratorError, 'return code 1, attempt 3/3'):
            self.check()
        self.assertEqual(self.runner.call_count, 3)
    ...
    def test_explicit_error_in_either_stream_stops_without_retry(self):
        for stdout, stderr in [('', 'fatal: not a git repository'), ('fatal: bad revision', ' \n')]:
            with self.subTest(stdout=stdout, stderr=stderr):
                ...
                with self.assertRaisesRegex(OrchestratorError, 'fatal:'):
                    self.check()
    ...
    def test_changed_head_after_retry_still_fails_closed(self):
        ...
        with self.assertRaisesRegex(OrchestratorError, 'agent changed Git history'):
            self.check()
    ...
    def test_timeout_or_start_failure_is_not_retried(self):
        self.runner.side_effect = OrchestratorError('command timed out')
        with self.assertRaisesRegex(OrchestratorError, 'timed out'):
            self.check()
        self.runner.assert_called_once()
        self.sleep.assert_not_called()
    ...
    def test_general_git_write_command_is_never_retried(self):
        ...
        with self.assertRaises(OrchestratorError):
            common.git(self.worktree, 'checkout', 'example')
        self.runner.assert_called_once()
        self.sleep.assert_not_called()
