...
|---|---|---|---|
| 2026-09-06 | next-day-setup | NDS hardening Phase 4.1(印刷fail-safeハードニング。印刷方式統合・`hotel_app.py`分割は対象外)。Phase 4監査で発見した3件に対応。(A)未知job keyのfail closed化: `print_jobs.py`に`UnknownPrintJobError`を追加し、`render_job`/`render_direct_preview`/`print_job`が未登録job key(または登録済みだが分岐の無いjob key)を明示的に拒否するよう変更。以前は`render_direct_preview()`が未知job keyを暗黙`else`で「伝票」として誤描画し、`print_job()`は逆に汎用Edge/HTML経路へ静かにfallbackしていた、その両方を閉じた。正式11帳票の挙動は無変更(全てのjob keyが既に明示分岐でカバーされていることをテストで確認済み)。(B)印刷UI経路6箇所(`preview_assignment_print`/`preview_kitchen_calendar`/`print_kitchen_calendar`/`preview()`/`print_one`/`print_all_jobs`/`print_configured_jobs`の非dinner分岐)の生traceback表示・無保護例外を、新設`HotelApp.summarize_print_failure`/`report_print_failure`経由の日本語要約表示＋`audit_event`への詳細記録に統一。**一括印刷の停止/継続という業務ポリシー自体は変更せず**(レビュー指摘を受けて誤って継続に変更していた箇所を停止方式へ復元)、表示方式のみ改善。(C)Add-Printerの複製プリンター名生成ロジック(`Ensure-DuplexPrinter`)を`Get-DuplexPrinterName`という印刷コマンドを一切含まない純粋関数へ切り出し、実際の`powershell.exe`でその抜粋のみを実行して検証(**Add-Printer自体の挙動は無変更**、Remove-Printer追加・確認ダイアログ・既存clone変更は今回も未対応)。(D)実印刷確認チェックリストを10項目から5項目へ再圧縮(夕食一括印刷1回でGDI複数用紙・両面・一括パイプライン・Excel COM+duplex clone・ケーキ帳票をまとめて確認)。テスト実装中、新設した監査ログ呼び出しが既存テストの未スタブにより実際の`operation_audit_2026-09-06.jsonl`へ2行書き込んでいたことを発見、原因(該当テストの`audit_event`未スタブ)を修正(実ファイルは編集せず内容のみ特定・報告)。回帰テスト33件追加。実プリンター・実PowerShell操作(印刷コマンド)・実Excel COM不要 | ローカルpytest 697 passed。GitHub Actions CI(`NDS pytest (Windows)`/`Dev standards`、push/pull_request両方)成功。差分8ファイルのみ確認、`origin/main`から behind 0 を確認。正式11帳票・Add-Printer挙動・一括印刷の停止/継続ポリシーがいずれも無変更であることを確認。PR #10をsquash merge(`a369735`)、作業ブランチ削除、正式ローカル同期、`check_standards.py`全10リポジトリOK、`DEV_DOCTOR` next-day-setupはup-to-date・未追跡0件、ERROR 0(他リポジトリの無関係なACTIONを除く) |
| 2026-09-06 | next-day-setup | NDS hardening Phase 4(印刷経路・Windows実機監査。印刷方式統合・`hotel_app.py`分割は対象外)。GDI直接印刷／Excel COM／reportlab+SumatraPDF／Edge HTMLの4方式が併存する印刷サブシステムを全棚卸し。`PRINT_JOBS`登録11帳票＋レジストリ外の独立2系統(ケーキ帳票Excel COM、設定可能なExcel自動印刷アイテム)を`docs/PHASE4_PRINT_AUDIT.md`として一覧化。print/preview分岐の不一致を発見: `render_direct_preview()`は未知job keyを暗黙`else`で「伝票」として誤描画、`print_job()`は逆に汎用Edge/HTML経路へ静かにfallbackする真逆の失敗モード。`kitchen_calendar`は一括印刷(reportlab PDF+SumatraPDF)と単体印刷/プレビュー(PIL画像+GDI)が同一job keyで完全に別実装であることも判明。死んだ印刷経路(`seating_chart`、`print_all`/`render_all`/`BULK_PRINT_KEYS`、未使用HTMLビルダー群、`print_one`とその呼び出し元一式)を発見・削除はせず記録。読み取り専用の`Get-Printer`診断で、`print_preparation.py`の`Ensure-DuplexPrinter`(Excel印刷の両面設定用に`Add-Printer`で複製プリンターを作成、対になる`Remove-Printer`は存在せず恒久的に残る)が**既に開発機で実行済みで合成プリンター`Codex_Duplex_Short/Long_Kyocera_TASKalfa_3554ci(J)_KX`が実在する**ことを確認(現行設定ファイルにも`duplex_mode: duplex_long`が1件設定済み)。実プリンター・PowerShell実行・Excel COM不要のテスト46件を追加(未テストだった8つの帳票描画関数の寸法・空データ・グリッド超過耐性、duplex_mode正規化のフォールバック境界、job key分岐の整合性)。実印刷・`Add-Printer`/`Remove-Printer`実行は一切なし | ローカルpytest 664 passed。GitHub Actions CI(`NDS pytest (Windows)`/`Dev standards`、push/pull_request両方)成功。差分4ファイル(監査ドキュメント+テスト3本)のみ確認、`origin/main`から behind 0 を確認。プリンター状態・印刷エンジン・`hotel_app.py`の挙動は無変更であることを確認。PR #9をsquash merge(`5955bf9`)、作業ブランチ削除、正式ローカル同期、`check_standards.py`全10リポジトリOK、`DEV_DOCTOR` next-day-setupはup-to-date・未追跡0件、ERROR 0/ACTION 0(他リポジトリの無関係なACTIONを除く) |
| 2026-09-06 | next-day-setup | NDS hardening Phase 3(日次データ整合性・復旧力の強化。SQLiteへの全面移行ではない)。`closing_tasks.py`の`load_scheduled_task_state`(`monthly_tasks.json`、旧形式含む)と`load_daily`(締め作業当日スナップショット)を、破損時に空状態へ静かにリセットしていた挙動から`ScheduledTaskStateLoadError`/`ClosingTaskDailyLoadError`を送出する設計へ変更(ファイル不存在=正常な「未保存」とは明確に区別)。保存は`atomic_write_json(backup=True)`化。実際の保存フロー(CSV取込→SQLite→日次JSON→席割/担当割/締め作業変更→再取込→終了→起動)を`docs/PHASE3_DATA_CONSISTENCY.md`として調査・文書化。日次JSONへ`kitchen_import_id`/`kitchen_imported_at`を埋め込み、起動時にSQLite側の最新取込と比較して`match`/`json_stale`/`sqlite_stale`/`no_snapshot`/`unknown`を判定、不一致は`messagebox.showwarning`のみ(自動上書き・自動修復は一切なし)。SQLiteに新テーブル`operator_state_backup`(既存DB非破壊の`CREATE TABLE IF NOT EXISTS`)を追加し、`seats`/`staff_assignments`/`closing_task_snapshot`のベストエフォート冗長コピーを保存(日次JSONが正本のまま、自動復元はしない)。最終監査として全リポジトリを対象に`load_scheduled_task_state`/`load_daily`の呼び出し元を追跡し、未保護だった2箇所(`print_closing_task_sheet`の単体印刷ボタン、`prepare_dinner_print_queue`の一括印刷締め作業分岐で生のトレースバックがダイアログに出る経路)を`kitchen_calendar`ジョブと同じパターンで修正。テスト58件追加(呼び出し元での単一ダイアログ・無変更ファイル・空データへのサイレント継続なしを確認する回帰テスト8件含む)。アプリ本体の業務ロジック・日次JSONフォーマット自体は変更なし | ローカルpytest 617 passed/1 skipped(環境依存)。GitHub Actions CI(`NDS pytest (Windows)`/`Dev standards`、push/pull_request両方)成功。差分11ファイルのみ確認、`origin/main`から behind 0 を確認。SQLiteスキーマ追加が既存DBを破壊しないことをシミュレートDBで確認。日次JSON=正本・SQLite=冗長コピーの位置づけと、自動復元ロジックが存在しないことをgrepで再確認。PR #8をsquash merge(`9606da9`)、作業ブランチ削除、正式ローカル同期、`check_standards.py`全10リポジトリOK、`DEV_DOCTOR` ERROR 0/ACTION 0 |
| 2026-09-06 | next-day-setup | NDS hardening Phase 2 第一段階(JSON保存・破損耐性)。全JSON I/O棚卸し(`docs/JSON_SAFETY_PHASE2.md`、重要度・正本orキャッシュ・SQLite重複・書き込み方式・読み込み失敗時の挙動を一覧化)。共通ヘルパー`json_safety.py`(`atomic_write_json`=同一ディレクトリ一時ファイル→flush→fsync→再パース検証→任意バックアップ→`os.replace`、`backup_existing_file`=世代数制限付きバックアップ、`quarantine_corrupted_file`=破損ファイル隔離)を新設。`master_settings.json`(最優先)は不存在/正常/壊れたJSON/型不正を区別し、壊れている場合は隔離保存+起動時警告(黙って初期化・上書きしない)。`load_work_data`は生のトレースバックの代わりに`WorkDataLoadError`(ファイル名・原因・バックアップからの復元手順を明記)を送出。`closing_tasks.json`の`save_master`もアトミック化。レビュー指摘で`__init__`内の旧設定キー移行処理の直接書き込みも統一。テスト48件追加。アプリ本体の業務ロジック・JSONフォーマット自体は変更なし | ローカルpytest 558 passed/2 skipped(環境依存)。GitHub Actions CI(push/pull_request両方)成功。CI限定のタイムスタンプ衝突flakeを発見しUUID付与で修正。実物の`master_settings.json`(28キー)・`closing_tasks.json`(18タスク)で警告ゼロ読み込みとアトミック書き込み往復一致を確認、既存データ形式との後方互換を実機確認。PR #7をsquash merge(`754d214`)、作業ブランチ削除、正式ローカル同期、`check_standards.py`全10リポジトリOK、`DEV_DOCTOR` ERROR 0/ACTION 0 |
| 2026-09-05 | next-day-setup | NDS hardening Phase 1(BUILD/配布経路の事故耐性)。`build_exe.py`にclean-tree gate(既定でdirty tree拒否、`--allow-dirty`は非常用override)、`update_shared_folder.ps1`に`Assert-SourceBuildInfo`(BUILD_INFO.txtの存在・EXE SHA-256一致・working tree=clean・記載HEADが現在の正式HEADと一致を配布前に検証、`-AllowDirtySource`/`-AllowStaleSource`で上書き可)、`Invoke-AtomicExeSwap`(同一ディレクトリ一時名コピー→SHA再照合→rename方式へ変更、直接`Copy-Item -Force`を廃止)を追加。ロールバックは既存バックアップを`-SourcePath`に指定し`-AllowStaleSource`付きで再実行するだけで可能に整理。GitHub Actions windows-latestでの`Get-FileHash`未解決問題を.NET直呼びハッシュへの置換で解消。テスト25件追加。アプリ本体の業務ロジックは変更なし | ローカルpytest 512件中511 passed/1 skipped(環境依存)。GitHub Actions CI(push/pull_request両方)成功、Get-FileHash関連skipゼロ。非本番ビルド・非本番デプロイ(H7方式)でdirty/clean・stale/正常・ロールバックの全パスをGit HEAD・EXE SHA-256一致まで検証。PR #6をsquash merge(`0d134be`)、作業ブランチ削除、正式ローカル同期、`check_standards.py`全10リポジトリOK、`DEV_DOCTOR` ERROR 0/ACTION 0 |
