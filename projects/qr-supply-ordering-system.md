...
一般物品を対象に、商品QRからの発注依頼と、発注先ごとのFAX準備・送信完了・履歴を管理する物品発注ドメイン。**在庫管理システムではない。**
...
| FAX | 次段階。業者別集約、個別送信 / 一斉送信、送信履歴を実装する方針。納品管理は行わない |
...
`最終発注日` はスマホからQR依頼を送った日時ではなく、PC側で業者へ実際にFAX送信が成功した日時を基準とする。
...
- 正式履歴は物品SQLite
- FAX送信後、スマホ表示に必要な各商品の最終発注日時をGoogle側商品ミラーへ反映する
...
QR依頼を検知した時点で即時自動FAXする方式は初期運用では採用しない。
数量間違い・誤操作・短時間に複数依頼が入るケースを人が確認できるよう、QR受付と実FAX送信の間に明示的な送信操作を残す。
...
各業者グループの下に `FAX送信` ボタンを置く。
画面全体には `未送信を一斉FAX` ボタンを置く。

一斉送信は業者単位で成功 / 失敗を独立管理し、途中で1社失敗しても他社の成功分を巻き戻さない。
失敗した業者だけ未送信として残し、再送できるようにする。

### 履歴

FAX送信成功時点を「実際の発注」とみなし、送信履歴をSQLiteへ保存する。
履歴では最低限以下を保持する。
...
旧物品Web側にも「FAX上の集約後も元依頼との対応を保持する」方針があるため、その境界は維持する。
...
将来、実FAX手段がAPIや自動送信可能なドライバ等で安定して制御できると確認できれば、自動FAXを追加する余地は残す。
ただし初期完成形は、

`QR自動受付 → 業者別自動集約 → 人が送信ボタン → 履歴自動保存`
...
QR受付が実機で通ったため、次はPC側のFAX処理を実装する。
実装順は、既存schema / FAX関連テーブルの再確認 → 業者別集約 → FAX帳票 / 送信手段 → 個別送信 / 一斉送信 → 送信履歴 → スマホ最終発注日反映とする。

実FAX送信手段は、現在使用している複合機 / FAXドライバ / サービスの仕様を確認してから決定し、架空の送信APIや成功判定を作らない。
...
同日、FAXは業者別自動集約 + 個別送信 / 一斉送信 + 送信履歴とし、納品管理は行わない方針を決定。スマホには継続状態ではなくFAX送信成功時点の `最終発注日` を表示する方針とした。
