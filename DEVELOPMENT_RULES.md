...
- Codexは、Windows実機、正式ローカルリポジトリ、ローカル専用ファイル、EXEビルド、実プリンター、共有サーバー、実機GUIなど、ChatGPTが直接扱えない環境の作業を担当する。
...
- Codexは、引き継がれたGitHub実装を無断で作り直さず、まず取得・ビルド・実機検証を行う。追加修正が必要な場合は差分理由を明示する。
- 本番共有フォルダへの反映、実運用データへの書き込み、実プリンター出力などは、従来どおり安全確認後に実施する。
...
- ChatGPT環境で実機確認できない項目は「未確認」と明示し、CodexまたはWindows実機で確認する。GitHub上の自動テスト成功を実機確認済みとして扱わない。
...
- Windows実機、EXE、プリンター、共有サーバーなどChatGPTから確認できない項目は未確認としてCodexへ引き継ぐ。
